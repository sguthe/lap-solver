...
template <class CF> void testRandom(long long min_tab, long long max_tab, bool omp, bool epsilon, std::string name_C);
template <class CF> void testGeometric(long long min_tab, long long max_tab, bool omp, bool epsilon, bool disjoint, std::string name_C);
template <class CF> void testGeometricCached(long long max_tab, long long min_cached, long long max_cached, bool omp, bool epsilon, bool disjoint, std::string name_C);
template <class CF> void testImages(std::vector<std::string> &images, long long max_tab, bool omp, bool epsilon, std::string name_C);
...
int main(int argc, char* argv[])
{
	...
	if (opt.use_double)
	{
			if (opt.use_single)
			{
				if (opt.run_random) testRandom<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, std::string("double"));
				if (opt.run_geometric) testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, false, std::string("double"));
				if (opt.run_geometric_disjoint) testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, true, std::string("double"));
				if (opt.run_geometric_cached) testGeometricCached<double>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, false, false, std::string("double"));
				if (opt.run_geometric_disjoint_cached) testGeometricCached<double>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, false, true, std::string("double"));
				if (opt.images.size() > 1) testImages<double>(opt.images, opt.lap_max_tab, opt.use_omp, false, std::string("double"));
			}
			if (opt.use_epsilon)
			{
				if (opt.run_random) testRandom<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, std::string("double"));
				if (opt.run_geometric) testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, false, std::string("double"));
				if (opt.run_geometric_disjoint) testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, true, std::string("double"));
				if (opt.run_geometric_cached) testGeometricCached<double>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, true, false, std::string("double"));
				if (opt.run_geometric_disjoint_cached) testGeometricCached<double>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, true, true, std::string("double"));
				if (opt.images.size() > 1) testImages<double>(opt.images, opt.lap_max_tab, opt.use_omp, true, std::string("double"));
			}
		}
	if (opt.use_float)
	{
		if (opt.use_single)
		{
			if (opt.run_random) testRandom<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, std::string("float"));
			if (opt.run_geometric) testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, false, std::string("float"));
			if (opt.run_geometric_disjoint) testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, false, true, std::string("float"));
			if (opt.run_geometric_cached) testGeometricCached<float>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, false, false, std::string("float"));
			if (opt.run_geometric_disjoint_cached) testGeometricCached<float>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, false, true, std::string("float"));
			if (opt.images.size() > 1) testImages<float>(opt.images, opt.lap_max_tab, opt.use_omp, false, std::string("float"));
		}
		if (opt.use_epsilon)
		{
			if (opt.run_random) testRandom<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, std::string("float"));
			if (opt.run_geometric) testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, false, std::string("float"));
			if (opt.run_geometric_disjoint) testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.use_omp, true, true, std::string("float"));
			if (opt.run_geometric_cached) testGeometricCached<float>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, true, false, std::string("float"));
			if (opt.run_geometric_disjoint_cached) testGeometricCached<float>(opt.lap_max_tab, opt.lap_min_cached, opt.lap_max_cached, opt.use_omp, true, true, std::string("float"));
			if (opt.images.size() > 1) testImages<float>(opt.images, opt.lap_max_tab, opt.use_omp, true, std::string("float"));
		}
	}
...
template <class C>
void testRandom(long long min_tab, long long max_tab, bool omp, bool epsilon, std::string name_C)
{
...
template <class C>
void testGeometric(long long min_tab, long long max_tab, bool omp, bool epsilon, bool disjoint, std::string name_C)
{
...
template <class C> 
void testGeometricCached(long long max_tab, long long min_cached, long long max_cached, bool omp, bool epsilon, bool disjoint, std::string name_C)
{
...
//				lap::omp::CachingIterator<C, C, decltype(costFunction), lap::omp::CacheSLRU> iterator(N, N, entries, costFunction, ws);
...
//				lap::omp::CachingIterator<C, C, decltype(costFunction), lap::omp::CacheLFU> iterator(N, N, entries, costFunction, ws);
				if (epsilon) costFunction.setInitialEpsilon(lap::omp::guessEpsilon<C>(N, N, iterator, (int)(N / entries)) / (disjoint ? C(10.0) : C(1000.0)));
				lap::displayTime(start_time, "setup complete", std::cout);
				lap::omp::solve<C>(N, costFunction, iterator, rowsol);
			}
...
template <class C> void testImages(std::vector<std::string> &images, long long max_tab, bool omp, bool epsilon, std::string name_C)
{
		...
		for (unsigned int b = a + 1; b < images.size(); b++)
		{
			...
			if (omp)
			{
				...
				if (N1 <= entries)
				{
					...
					if (epsilon) costMatrix.setInitialEpsilon(eps_factor);//lap::omp::guessEpsilon<C>(N1, N2, costMatrix) / eps_factor);
...
//						lap::omp::CachingIterator<C, C, decltype(costFunction), lap::omp::CacheSLRU> iterator(N1, N2, (int)entries, costFunction, ws);
						if (epsilon) costFunction.setInitialEpsilon(eps_factor);//llap::omp::guessEpsilon<C>(N1, N2, costFunction, (int)(N1 / entries)) / eps_factor);
						lap::displayTime(start_time, "setup complete", std::cout);
...
//						lap::omp::CachingIterator<C, C, decltype(costFunction), lap::omp::CacheLFU> iterator(N1, N2, (int)entries, costFunction, ws);
						if (epsilon) costFunction.setInitialEpsilon(eps_factor);//llap::omp::guessEpsilon<C>(N1, N2, costFunction, (int)(N1 / entries)) / eps_factor);
						lap::displayTime(start_time, "setup complete", std::cout);
						lap::omp::solve<C>(N1, N2, costFunction, iterator, rowsol);
			...
			else
			{
				...
				if (N1 <= entries)
				{
					...
					if (epsilon) costMatrix.setInitialEpsilon(eps_factor);//llap::guessEpsilon<C>(N1, N2, costMatrix) / eps_factor);
				...
				else
				{
					...
					if (4 * entries < N1)
					{
						...
						if (epsilon) costFunction.setInitialEpsilon(eps_factor);//llap::guessEpsilon<C>(N1, N2, costFunction, (int)(N1 / entries)) / eps_factor);
						lap::displayTime(start_time, "setup complete", std::cout);
